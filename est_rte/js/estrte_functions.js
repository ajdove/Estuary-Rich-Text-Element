...
inputHtml = inputHtmlRaw.replace(/[&<>"']/g, function(m) { return map[m]; });
...
function estrte_add_style(styleToAdd, newValue){
	...
	if(estrte_fragments_log.length > 0){
       ...
       let origInputText = document.getElementById("estrte_content_wrapper").innerHTML;
	}
    const date = new Date();
...
document.getElementById("format" + styleToAdd). style.fontWeight ="normal";
            ...
            el.id = unix;
            el.className = "inserted_div";
            let docFrag = document.createDocumentFragment();  // The createDocumentFragment() method creates an offscreen node.
            node = el;
              lastNode = docFrag.appendChild(node);
...
blue = 255 -((225 - tempBlue) * (2 - shade)); 
	...
	if(estrte_fragments_log.length > 0){
       ...
       let origInputText = document.getElementById("estrte_content_wrapper").innerHTML;
                    ...
                    let date = new Date();
let unix = Math.round(+date / 1000) + Math.floor(Math.random() * 1000);
...
estrte_to_focus = "estrte_input_field";
				}
				}
...
document.getElementById("estrte_input_field").contentEditable = false;
   inserted_divs = document.getElementsByClassName("inserted_div");
        ...
        for(i = 0; i < inserted_divsLength; i++){
         ...
         let inserted_elements = document.getElementsByClassName("inserted_div");
...
existingSegment = selectedTextString;
				...
				sel = window.getSelection();
            el = document.createElement("div");
            let table_html = '<div class="table_spec_form" id="table_spec_form"><div id="table_spec_heading">Table Properties<span class="close_window" title="Close" onclick="close_add_table()">x</span></div>';
...
existingSegment = selectedTextString;
				...
				sel = window.getSelection();
            el = document.createElement("div");
            let link_html = '<div class="table_spec_form" id="link_spec_form"><div id="table_spec_heading">Add Link<span class="close_window" title="Close" onclick="close_add_link()">x</span></div>';
...
document.getElementById("estrte_select_color_div").innerHTML = estrte_select_colour_div_html;
	    ...
	    $("#estrte_select_color_label").off("touchenter");
	    $("#estrte_select_color_label").off("onmouseover");
 }
...
function estrte_show_select_special_characters_div(){
    estrte_color_editing = false;
	    ...
	    $("#estrte_select_special_characters_label").off("touchenter");
	    $("#estrte_select_special_characters_label").off("onmouseover");
}
...
// 	estrte_action = "set_background";
 ...
 {
    let shade = document.getElementById("shadeComp").value / 100;
        if(document.getElementById("tempRed").value != ''){
    	let redValue = document.getElementById("tempRed").value;
    }else{
    	let redValue = document.getElementById("redComp").value;
    }
    if(document.getElementById("tempGreen").value != ''){
    	let greenValue = document.getElementById("tempGreen").value;
    }else{
    	let greenValue = document.getElementById("greenComp").value;
    }
    if(document.getElementById("tempBlue").value != ''){
    	let blueValue = document.getElementById("tempBlue").value;
    }else{
    	let blueValue = document.getElementById("blueComp").value;
    }
	document.getElementById("tempRed").value = redValue;
 	document.getElementById("tempGreen").value = greenValue;
 	document.getElementById("tempBlue").value = blueValue; 
